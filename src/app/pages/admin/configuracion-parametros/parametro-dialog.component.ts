...
import { mostrarToastWarning, mostrarToastExito, mostrarToastError } from '../../../core/utils/swal.util';
...
export class ParametroDialogComponent implements OnInit {
  ...
  guardar(): void {
    ...
    request$
      ...
      .subscribe({
        next: () => {
          mostrarToastExito(this.data.mode === 'edit' ? 'Parámetro actualizado' : 'Parámetro creado');
          this.dialogRef.close(true);
        },
        error: (error) => {
          mostrarToastError('Error al guardar', this.resolveErrorMessage(error, 'No se pudo guardar el parámetro. Intente nuevamente más tarde.'));
        }
      });
  }
